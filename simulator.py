...
from event import (EventJobComplete, EventReAlloc, EventStageSubmit,
                   EventJobSubmit, EventStageComplete, EventTaskComplete,
                   EventTaskSubmit)
    ...
    def run(self):
        ...
        while not self.event_queue.empty():
            ...
            if isinstance(event, EventReAlloc):
                ...
                if len(self.cluster.finished_jobs) < len(self.job_list[0]):  # TODO job_list[0]
            ...
            # 生成对映的 Stages 并添加 StageSubmit
                ...
                current_job_index[event.job.user_id] = event.job.index  # TODO 干嘛的
            ...
            # 看来来好像没用
                ...
                if self.cluster.isDebug:
                    print("time", event.time,
                          "-submit task-", event.task.id,
                          "-job-", event.task.job_id,
                          "-slot-", event.task.machine_id)
            ...
            # 添加对映的 StageSubmit 和 JobCompete
                ...
                if event.stage.job.service_type == self.cluster.foreground_type:
                    ...
                    print("stage finish: ", event.stage.id,
                          "used slots number:", len(stageSlots),
                          "submit interval", event.stage.last_task_submit_time - event.stage.submit_time,
                          "currently reserved for this job:", self.cluster.jobIdToReservedNumber[event.stage.job.id])
            ...
            # JobComplete
                ...
                print("-", event.job.id,
                      " (job) finishes, duration", event.job.duration,
                      " job.alloc ", event.job.alloc,
                      "PR:", float(event.job.monopolize_time) / event.job.execution_time)
