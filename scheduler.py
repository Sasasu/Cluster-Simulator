class Scheduler:
        ...
        for stage in self.task_buffer:
            ...
            for task in tmpList:
                if self.cluster.isDebug:
                    print(time, " make_offers():", len(self.cluster.make_offers()),
                          "open number:", self.cluster.open_machine_number,
                          "task-jobid:", task.job_id, len(self.cluster.jobIdToReservedNumber))
                ...
                for machineId in self.cluster.make_offers():
                    print("taskid", task.id,
                          "job", task.job_id,
                          "machineid", machineId,
                          "is_reserved", self.cluster.machines[machineId].is_reserved)

                    if self.cluster.machines[machineId].is_reserved != -1 and \
                            task.job_id != self.cluster.machines[machineId].is_reserved:
                        continue
                    sign = True
                ...
                if self.cluster.isDebug:
                    print(time, task.id, task.stage_id, "success:", success,
                          "sign:", sign,
                          "len:", len(stage.not_submitted_tasks))
        return msg
    ...
    def submit_stage(self, stage, time):
        ...
        if len(stage.parent_ids) == 0:
            self.stageIdToAllowedMachineId[stage.id] = list(range(self.cluster.machine_number))
        else:
            tmpList = list()
    ...
    def stage_complete(self, stage):
        ...
        if stage.job.service_type == 0:
            ...
            print("stage complete:", stage.id,
                  "stage tasknum:", len(stage.taskset),
                  "used machine number:", len(machinelist))
        return msg
