class Job:
    ...
    def set_curve(self, curveString):
        ...
        self.demand = float(len(self.curve) - 1)  #  重复实验至少需要测出完全使用集群时的JCT, curve 是离散的 len(curve) = 最大需要的服务器数量
    ...
    def get_min_alloc(self, alpha):
        tmp_i = 1
        while tmp_i <= int(self.fairAlloc) and \
                self.curve[int(self.fairAlloc) - tmp_i] >= self.curve[int(self.fairAlloc)] * alpha:
            tmp_i += 1
